...
def google_status(admin: AdminClient, current_user: CurrentUser):
    row = admin.table("user_integrations").select("connected_at").eq("user_id", current_user.id).eq("provider", "google_calendar").maybe_single().execute().data
...
def google_callback(code: str, state: str, admin: AdminClient):
    match = admin.table("integration_link_codes").select("*").eq("provider", "google_calendar").eq("code", state).maybe_single().execute().data
    ...
    existing = admin.table("user_integrations").select("refresh_token").eq("user_id", match["user_id"]).eq("provider", "google_calendar").maybe_single().execute().data
