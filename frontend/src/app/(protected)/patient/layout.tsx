    ...
    <div className="min-h-screen bg-slate-50 text-navy-900">
      <header className="sticky top-0 z-40 border-b border-border bg-white/95 backdrop-blur">
        <div className="mx-auto flex min-h-16 items-center gap-4 px-5 lg:hidden">
          <Link href="/patient/dashboard" className="flex shrink-0 items-center rounded-2xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500">
        ...
        {isMobileMenuOpen ? (
          <div className="border-t border-blue-100 bg-white px-5 pb-5 pt-3 shadow-[0_14px_30px_rgba(20,46,86,0.12)] lg:hidden">
            <div className="mb-3 rounded-2xl bg-blue-50 px-4 py-3">
              <p className="truncate font-bold text-navy-900">{user?.name || "ผู้ป่วย"}</p>
            ...
            <nav className="grid gap-1" aria-label="เมนูมือถือ">
              <Link href="/patient/profile" className="flex min-h-12 items-center gap-3 rounded-xl px-4 font-semibold text-slate-600 transition hover:bg-blue-50">
                <TablerIcon name="user" className="h-5 w-5" />
                โปรไฟล์
              </Link>
              <Link href="/patient/caregivers" className="flex min-h-12 items-center gap-3 rounded-xl px-4 font-semibold text-slate-600 transition hover:bg-blue-50">
          ...
          <div className="hidden shrink-0 items-center gap-3 lg:flex">
            <details ref={profileMenuRef} className="group relative">
              <summary className="inline-flex min-h-10 cursor-pointer list-none items-center gap-3 rounded-full bg-slate-100 pl-1 pr-3 text-sm font-semibold text-navy-900 transition hover:bg-blue-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 [&::-webkit-details-marker]:hidden">
                ...
                <span className="max-w-28 truncate">{user?.name || "ผู้ป่วย"}</span>
              </summary>
              ...
              <div className="absolute right-0 top-[calc(100%+12px)] hidden w-80 overflow-hidden rounded-[18px] border border-border bg-white shadow-[0_16px_38px_rgba(20,46,86,0.14)] group-open:block">
                <div className="border-b border-border px-5 py-4">
                  <p className="truncate text-lg font-bold text-navy-900">{user?.name || "ผู้ป่วย"}</p>
                  <p className="mt-1 truncate text-sm text-slate-500">{user?.email || "กำลังโหลดข้อมูล..."}</p>
                </div>
                <Link href="/patient/profile" className="flex min-h-12 items-center gap-3 px-5 font-semibold text-slate-700 transition hover:bg-blue-100">
                  <TablerIcon name="user" className="h-5 w-5" />
                  โปรไฟล์
                </Link>
      ...
      {showSwitchModal ? (
        <div className="fixed inset-0 z-[60] grid place-items-center bg-slate-950/55 px-4 backdrop-blur-[1px]" role="dialog" aria-modal="true" aria-labelledby="switch-role-title">
          <div className="w-full max-w-[560px] rounded-[18px] bg-white p-7 shadow-[0_24px_70px_rgba(15,23,42,0.24)]">
            <h2 id="switch-role-title" className="text-2xl font-bold text-slate-950">สลับไปยัง โหมดผู้ดูแล?</h2>
            <p className="mt-4 text-base font-medium text-slate-500">คุณจะถูกสลับไปยัง โหมดผู้ดูแล</p>
      ...
      <nav className="fixed inset-x-8 bottom-4 z-50 grid grid-cols-3 gap-1 rounded-[22px] border border-border bg-white/95 p-1.5 shadow-[0_8px_24px_rgba(20,46,86,0.14)] backdrop-blur lg:hidden" aria-label="เมนูหลัก">
        {navItems.map((item) => (
          <Link
            key={item.href}
            href={item.href}
